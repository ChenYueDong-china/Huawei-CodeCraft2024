...
public class Strategy {
    ...
    public final HashSet<Integer> berthLock = new HashSet<>();//锁住某些工作台
...
//        }
    ...
    public ArrayList<Point> robotToAnyPoint(Robot robot, Point target, ArrayList<ArrayList<Point>> otherPath, boolean[][] conflictPoints) {
        short maxDeep = Short.MAX_VALUE;
        return robotMoveToPointBerth(gameMap, robot.pos, -1, target, maxDeep, otherPath,
                null, conflictPoints);
    }
    ...
    public ArrayList<Point> robotToBerthHeuristic(Robot robot, int berthId, ArrayList<ArrayList<Point>> otherPath, boolean[][] conflictPoints) {
        ...
        if (conflictPoints != null) {
            maxDeep += (short) (ROBOT_AVOID_OTHER_DEEP);//多5帧能让对面也行,不然只能随机了
        }
...
//        }
        return robotMoveToPointBerthHeuristicCs(gameMap, robot.pos, berthId, null, maxDeep, otherPath
                , berths.get(berthId).robotMinDistance, conflictPoints); //考虑
    ...
    public void mainLoop() throws IOException, InterruptedException {
        ...
        printError("jumpTime:" + jumpCount + ",buyRobotCount:" + robots.size() + ",buyBoatCount:" + boats.size() + ",totalValue:"
                + totalValue + ",pullValue:" + pullScore + ",score:" + money + ",boatSellCount:" + sellCount);
    ...
    private void robotAndBoatBuy() {
        ...
        int robotTotalSize = robots.size() + pendingRobots.size();
        int type0Size = 0;
        int type1Size = 0;
        ...
        int remainCount = 0;
        for (Workbench2 value : workbenches.values()) {
    ...
    private void buyRobot(int type) {
        int index = getBestBuyRobotPos();
        outStream.printf("lbot %d %d %d\n", robotPurchasePoint.get(index).x, robotPurchasePoint.get(index).y, type);
    ...
    private void boatDoAction() {
        ...
        for (Berth berth : berths) {
            ...
            if (berth.curBoatGlobalId != -1) {
                SimpleBoat boat = totalBoats.get(berth.curBoatGlobalId);
                if (boat.belongToMe) {
                    continue;
                }
                int needCount = max(0, boatCapacity - boat.num);
        ...
        for (Boat boat : tmpBoats) {
            ...
            if (boat.carry) {
                ...
                if (berth.curBoatGlobalId != -1 && berth.curBoatGlobalId != boat.globalId) {
                    for (int i = 0; i < min(boat.path.size(), 5); i++) {
                        PointWithDirection next = boat.path.get(i);
                        if (next.point.equal(berth.corePoint) && !boat.corePoint.equal(berth.corePoint)) {
                            //有船且不是你,先去核心点等着闪现
        ...
        for (Boat boat : boats) {
            ...
            for (SimpleBoat other : totalBoats) {
                ...
                //撞到一个超过五帧不动的人,
                if (other.noMoveTime > 5 && boatCheckCrash(gameMap, next, other.pointWithDirection)) {
                    avoidOther = true;
                    break;
                }
            }
        ...
        printError("frame:" + frameId + ",BoatDi:" + (l2 - l1) + "，path：" + (l3 - l2) + ",avoid:" + (l4 - l3) + ",avoidOther:" + (l5 - l4));
    }
...
//                , maxDeep, boat.id, otherPaths, otherIds, heuristicCs, conflictPoints);
    ...
    private boolean boatGreedySell() {
        ...
        for (Boat boat : boats) {
            ...
            short minSellDistance = Short.MAX_VALUE;
            BoatSellPoint selectPoint = null;
            for (BoatSellPoint boatSellPoint : boatSellPoints) {
                if (!boatSellPoint.canReach(boat.corePoint, boat.direction)) {
                    continue;
                }
                short distance = boatSellPoint.getMinDistance(boat.corePoint, boat.direction);
                //避让时间,稍微增加一帧
                distance += (short) boat.remainRecoveryTime;
    ...
    private boolean boatGreedyBuy1(int[] goodsNumList) {
        ...
        for (Berth berth : berths) {
            //停靠在泊位上了
            if (berth.curBoatGlobalId != -1
                    && berth.goodsNums > 0 && totalBoats.get(berth.curBoatGlobalId).belongToMe
                    && !boats.get(totalBoats.get(berth.curBoatGlobalId).localId).buyAssign
                    && !boats.get(totalBoats.get(berth.curBoatGlobalId).localId).carry) {
                //没去卖
                Boat boat = boats.get(totalBoats.get(berth.curBoatGlobalId).localId);
        ...
        for (int i = 0; i < berths.size(); i++) {
            ...
            if (berthLock.contains(berth.id)) {
                continue;
            }
            ...
            for (BoatSellPoint boatSellPoint : boatSellPoints) {
                short distance = boatSellPoint.getMinDistance(berth.corePoint, berth.coreDirection);
                if (distance < minSellDistance) {
                    minSellDistance = distance;
            ...
            if (!selectBoat.carry
                    ...
                    berths.get(selectBoat.targetBerthId).curBoatGlobalId
                            != selectBoat.globalId) {
                //没到泊位切换目标降低点价值
        ...
        if (!bestBoat.carry && bestBoat.targetBerthId != -1 &&
                berths.get(bestBoat.targetBerthId).curBoatGlobalId == bestBoat.globalId &&
                bestBerth.id != bestBoat.targetBerthId &&
                goodsNumList[bestBerth.id] < BOAT_CHANGE_BERTH_MIN_NUM_DIFF) {
            bestBerth = berths.get(bestBoat.targetBerthId);
        }
...
//
    private void robotDoAction() {
        ...
        for (Robot robot : robots) {
            ...
            boolean avoidOther = false;
            Point originNext = gameMap.discreteToPos(robot.path.get(2));
            if (!originNext.equal(robot.pos)) {
                for (SimpleRobot other : totalRobots) {
                    if (other.belongToMe) {
                        continue;
                    }
                    if (other.noMoveTime > ROBOT_DIRECTION_AVOID_OTHER_TRIGGER_THRESHOLD && other.p.equal(originNext)) {
                        avoidOther = true;
                        break;
                    }
                }
            ...
            ArrayList<Point> avoidPoints = new ArrayList<>();
            avoidPoints.add(originNext);//前一个点是避让点
            for (int j = 0; j < DIR.length / 2; j++) {
                Point e = robot.pos.add(DIR[j]);
                if (e.equal(originNext) ||
        ...
        long l3 = System.currentTimeMillis();
        printError("frame:" + frameId + ",robotDecision:" + (l2 - l1) + ",robotOther:" + (l3 - l2));
    }
    ...
    private ArrayList<Point> robotToWorkBenchHeuristic(Robot robot, int targetWorkBenchId, ArrayList<ArrayList<Point>> otherPaths, boolean[][] conflictPoints) {
        ...
        if (conflictPoints != null) {
            maxDeep += (short) (ROBOT_AVOID_OTHER_DEEP);
        }
...
//
    ...
    private boolean greedyBuy() {
        ...
        for (Workbench2 buyWorkbench : workbenches.values()) {
            ...
            if (selectRobot == null) {
                buyWorkbench.curMaxProfitRobotId = -2;
                continue;
            }
            ...
            //保存剪枝
            buyWorkbench.curMaxProfitRobotId = selectRobot.id;
    ...
    private boolean greedySell() {
        ...
        for (Robot robot : robots) {
            ...
            for (Berth sellBerth : berths) {
                ...
                if (berthLock.contains(sellBerth.id)) {
                    continue;
                }
...
//
    ...
    private boolean input() throws IOException {
        ...
        for (int i = 0; i < totalRobotsCount; i++) {
            ...
            simpleRobot.input();
            //5帧不动且是贵重物品
            if (simpleRobot.noMoveTime > ROBOT_DIRECTION_AVOID_OTHER_TRIGGER_THRESHOLD && !simpleRobot.belongToMe) {
        ...
        for (SimpleBoat boat : totalBoats) {
            ...
            if (boat.status == 2) {
                ...
                assert berthId != -1;
                berths.get(berthId).curBoatGlobalId = boat.id;//一定进入泊位
            }
        }
        ...
        if (BOATS_PER_PLAYER != 0) {
            ...
            for (int i = 0; i < BOATS_PER_PLAYER; i++) {
                int id = Integer.parseInt(parts[i]);
                totalBoats.get(id).belongToMe = true;
                Boat boat = boats.get(i);
                boat.id = i;
                boat.globalId = id;
                totalBoats.get(id).localId = i;
